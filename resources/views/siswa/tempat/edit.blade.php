...
<body class="bg-gray-50 text-gray-800">
    <div class="container mx-auto p-6 max-w-lg">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            <form
                action="{{ $isAdmin ? route('admin.tempat.update', $tempat->id) : route('siswa.tempat.update', $tempat->id) }}"
                ...
                <div>
                    ...
                    <input type="text" name="instruktur_perusahaan"
                        value="{{ old('instruktur_perusahaan', $tempat->instruktur_perusahaan) }}"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1d9a96]">
                </div>

                @if ($isAdmin)
                    <div>
                        <label class="block font-medium">Guru Pembimbing</label>
                        <select name="guru_id" class="w-full px-4 py-2 border rounded-lg">
                            <option value="">-- Pilih Guru --</option>
                            @foreach ($gurus as $g)
                                <option value="{{ $g->id }}" {{ $tempat->guru_id == $g->id ? 'selected' : '' }}>
                                    {{ $g->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="belum_terverifikasi"
                                {{ $tempat->status == 'belum_terverifikasi' ? 'selected' : '' }}>Belum Terverifikasi
                            </option>
                            <option value="proses" {{ $tempat->status == 'proses' ? 'selected' : '' }}>Proses</option>
                            <option value="diterima" {{ $tempat->status == 'diterima' ? 'selected' : '' }}>Diterima
                            </option>
                            <option value="ditolak" {{ $tempat->status == 'ditolak' ? 'selected' : '' }}>Ditolak
                            </option>
                        </select>
                    </div>
                @endif
